...
exports.updateUserEmployeeNo = async (req, res) => {
  try {
    ...
    if (Gen_Manu_mem_No === undefined) {
        return res.status(400).json({ message: 'Employee number (Gen_Manu_mem_No) is required.' });
    }
    ...
    if (existingRecord.recordset.length > 0) {
      ...
      await pool.request()
        ...
        .query('UPDATE Gen_Manu_Member SET Gen_Manu_mem_No = @no WHERE Gen_Manu_mem_Memid = @id');
    } else {
      // ถ้าไม่มี, ให้เพิ่มใหม่ (INSERT)
      // 1. ดึงข้อมูลที่จำเป็นทั้งหมดจาก agt_member พร้อม JOIN
      const memberDataResult = await pool.request()
        .input('id', sql.NVarChar, id)
        .query(`
          SELECT 
            m.agt_member_nameEN,
      ...
      if (memberDataResult.recordset.length === 0) {
          return res.status(404).json({ message: 'Member not found in agt_member table.'});
      }
      ...
      await pool.request()
        ...
        .input('shift', sql.NVarChar, memberData.agt_member_shift)
        .query(`
    ...
    res.status(500).json({ message: 'Error updating employee number', error: error.message });
  }
};