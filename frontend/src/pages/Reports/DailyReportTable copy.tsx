...
const DailyReportTable: React.FC<DailyReportTableProps> = ({ data, onUpdateStPlan, selectedDate, selectedLotNo, mode = 'normal', hideZE1A = false }) => {
  ...
  if (mode === 'ze1a') {
    return (
      <div className="w-full bg-white border border-gray-400 shadow-sm overflow-hidden">
        {/* ไม่มีปุ่ม Save Summary เพราะใช้ร่วมกับหน้า 1 หรือถ้าอยากมีก็ใส่ได้ */}
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr className="bg-slate-900 text-white">
                <th className="border-r border-gray-600 p-1 text-center font-bold text-xs w-8">No.</th>
                <th className="p-1 text-center font-bold text-sm uppercase tracking-widest" colSpan={6}>
                  Genmatsu ZE-1A
                </th>
              </tr>
              <tr className="border-b border-gray-300">
                <th className="border-r border-b border-gray-300 bg-gray-50"></th>
                <TableHeaderGroup title="Line ZE-1A" hasMoisture={true} />
              </tr>
            </thead>
              ...
              {maxRows === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-6">
                    <span className="text-sm text-gray-400">No ZE-1A Data Available</span>
                  </td>
                </tr>
              ) : (
                <>
                  {Array.from({ length: maxRows }).map((_, index) => (
                    <tr key={index} className="hover:bg-blue-50 border-b border-gray-300 transition-colors">
                      ...
                      {renderDataCells(data.lineZE1A ? data.lineZE1A[index] : undefined, true)}
                    </tr>
                  ))}
                  {/* Total Row for ZE-1A */}
                  <tr className="border-t-2 border-gray-500 bg-gray-100 font-bold">
                    ...
                    {renderTotalCells(data.lineZE1A || [], true)}
                  </tr>
                </>
              )}
            </tbody>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse text-xs">
          ...
          <tbody className="bg-white">
            {maxRows === 0 ? (
              <tr>
                {/* ✅ ในโหมดปกติ ใส่ 19 ไปเลยครับ ไม่ต้องเช็ค mode แล้ว */}
        ...
        <div className="mt-8 border-t-4 border-gray-500 pt-4">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-xs">
              ...
              <tbody className="bg-white">
                {Array.from({ length: data.lineZE1A.length }).map((_, index) => (
                  <tr key={index} className="hover:bg-blue-50 border-b border-gray-300 transition-colors">
                    <td className="border-r border-gray-300 p-0.5 text-center font-bold text-gray-600 bg-gray-50 align-top text-xs">
                      {index + 1}
                    </td>
