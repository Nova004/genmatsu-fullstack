...
const ReportHistory_Gen_A: React.FC = () => {
    ...
    useEffect(() => {
        const fetchSubmissions = async () => {
            try {
                // บอก TypeScript ว่าข้อมูลที่ได้มาจะเป็นหน้าตาแบบ SubmissionData[]
                const data: SubmissionData[] = await getAllSubmissions();
                const filteredData = data.filter(sub => sub.form_type === 'GEN_A');
    ...
    const columns = useMemo<ColumnDef<SubmissionData>[]>(
        () => [
            {
                accessorKey: 'submitted_by_name', // 👈 แก้ไขตรงนี้
                header: 'ผู้บันทึก',
            },
            {
                accessorKey: 'lot_no',
    ...
    return (
        <>
            <Breadcrumb pageName="ประวัติการบันทึก (Genmatsu A)" />
