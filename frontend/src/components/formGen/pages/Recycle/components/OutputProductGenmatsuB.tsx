...
interface OutputProductGenmatsuBProps {
...
const OutputProductGenmatsuB: React.FC<OutputProductGenmatsuBProps> = ({
  register,
  watch,
  ...
  const totalWeightLeft = itemsLeft.reduce((sum: number, item: any) => {
    const weight = parseFloat(item?.weight) || 0;
    return sum + weight;
  }, 0);

  const totalWeightRight = itemsRight.reduce((sum: number, item: any) => {
    const weight = parseFloat(item?.weight) || 0;
    return sum + weight;
  }, 0);

  const totalWeight = totalWeightLeft + totalWeightRight;

  const handleAddRow = () => {
    append({ bagNo: '', weight: undefined });
    appendRight({ bagNo: '', weight: undefined });
  };
        ...
        <table className="w-full table-auto">
              ...
              <tr key={field.id}>
                ...
                <td className={tdCenterClass}>
                  <input
                    ...
                    disabled
                    {...register(`${fieldName}Right.${index}.bagNo`)}
                  />
                </td>
                ...
                <td className={tdCenterClass}>
                  <input
                    ...
                    className={inputClass}
                    placeholder=""
                    {...register(`${fieldName}Right.${index}.weight`, {
                      setValueAs: (value) => value === '' ? undefined : parseFloat(value)
                    })}
                  />
                </td>
          ...
          <tfoot>
            <tr className="bg-gray-2 dark:bg-meta-4">
              <td className={footerTdClass} colSpan={3}>Total</td>
...
export default OutputProductGenmatsuB;
