...
// --- Component ย่อยสำหรับส่วน Validation (ฉบับอัปเกรด) ---
    ...
    return (
        <div className="mt-4 rounded-md border border-stroke p-4 dark:border-strokedark">
            <div className="flex items-center justify-between">
                ...
                <button 
                    onClick={() => onConfigChange(path, undefined)} // ส่ง undefined เพื่อลบ key นี้
...
const EditItemModal: React.FC<Props> = ({ isOpen, item, onClose, onSave }) => {
  ...
  const handleAddValidation = (path: string) => {
    ...
    handleConfigChange(path, newValidationRule);
  };

  const renderEditForm = () => {
    // --- Logic สำหรับ Template ของ Step 2 ---
    if ('row_type' in config) {
      ...
      return (
        <div className="flex flex-col gap-6">
          ...
          <div>
            {step2Config.validation ? (
               <ValidationFields validation={step2Config.validation} path="validation" onConfigChange={handleConfigChange} />
            ) : (
              <button onClick={() => handleAddValidation('validation')} className="w-full rounded border border-dashed py-2 text-primary hover:bg-gray-100 dark:hover:bg-meta-4">
                  + Add Top-Level Validation
              </button>
            )}
          </div>
          ...
          {step2Config.inputs?.map((input, index) => (
            <div key={index} className="rounded-md border border-stroke p-4 dark:border-strokedark">
                ...
                {input.validation ? (
                    ...
                    <button onClick={() => handleAddValidation(`inputs.${index}.validation`)} className="mt-4 w-full rounded border border-dashed py-2 text-primary hover:bg-gray-100 dark:hover:bg-meta-4">
    ...
    // --- Logic สำหรับ Template ของ Step 3 ---
              ...
              {column.type === 'SINGLE_INPUT_GROUP' && column.input && (
                <>
                  ...
                  {column.input.validation ? (
                    ...
                    <button onClick={() => handleAddValidation(`columns.${index}.input.validation`)} className="mt-4 w-full rounded border border-dashed py-2 text-primary hover:bg-gray-100 dark:hover:bg-meta-4">
                      + Add Validation
                    </button>
                  )}
                </>
                ...
                <>
                  <FormInput label="Row Description" value={column.description} onChange={e => handleConfigChange(`columns.${index}.description`, e.target.value)} />
                   {column.validation ? (
                        ...
                        <button onClick={() => handleAddValidation(`columns.${index}.validation`)} className="mt-4 w-full rounded border border-dashed py-2 text-primary hover:bg-gray-100 dark:hover:bg-meta-4">
