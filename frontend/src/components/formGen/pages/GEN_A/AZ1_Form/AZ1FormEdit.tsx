...
const AZ1FormEdit: React.FC<AZ1FormEditProps> = ({ initialData, onSubmit, onResubmit, submissionId, status, templates }) => {
    ...
    const { step, setStep, handleNext, handleBack, handleSubmit_form } = useMultiStepForm({
        ...
        trigger,
        errors,
        validationSchema: AZ1_VALIDATION_SCHEMA,
    });
    // --- ค่าคงที่สำหรับ Styling และ Dropdown ---
    const availableForms = [{ value: 'G022', label: 'AZ1', path: '#' }]; // ไม่จำเป็นต้องมี path จริงในโหมดแก้ไข
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6">
            <form onSubmit={handleSubmit(handleFormSubmit)}>
                <FormHeader
                    title="แก้ไขใบรายงานการผลิต (AZ1)" // เปลี่ยน Title สำหรับหน้าแก้ไข
                    ...
                    */}
                    ...
                    {step === 2 && (
                        <FormStep2
                            ...
                            staticBlueprint={status === 'Approved' && templates ? templates['AZ1_Step2_RawMaterials'] : undefined}
                        />
                    )}
                    {step === 3 && (
                        <SharedFormStep3
                            ...
                            getValues={getValues}
                            onTemplateLoaded={() => { }}
                            templateName="AZ1_Step3_Operations"
                            // 👇 Pass logic Approved ? Old : New
                            staticBlueprint={status === 'Approved' && templates ? templates['AZ1_Step3_Operations'] : undefined}
                        />
                    )}
