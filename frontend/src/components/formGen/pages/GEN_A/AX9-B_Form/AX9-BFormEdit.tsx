// location: frontend/src/components/formGen/pages/AX9-B_Form/AX9-BFormEdit.tsx
...
const AX9_BFormEdit: React.FC<AX9_BFormEditProps> = ({ initialData, onSubmit, onResubmit, submissionId, status, templates }) => {
    ...
    const { step, setStep, handleNext, handleBack, handleSubmit_form } = useMultiStepForm({
        ...
        trigger,
        errors,
        validationSchema: AX9_B_VALIDATION_SCHEMA,
    });


    // --- ค่าคงที่สำหรับ Styling และ Dropdown ---
    const availableForms = [{ value: 'G021', label: 'AX9-B', path: '#' }]; // ไม่จำเป็นต้องมี path จริงในโหมดแก้ไข
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6">
            <form onSubmit={handleSubmit(handleFormSubmit)}>
                <FormHeader
                    title="แก้ไขใบรายงานการผลิต (AX9-B)" // เปลี่ยน Title สำหรับหน้าแก้ไข
                    ...
                    */}
                    ...
                    {step === 2 && (
                        <FormStep2
                            ...
                            staticBlueprint={status === 'Approved' && templates ? templates['AX9-B_Step2_RawMaterials'] : undefined}
                        />
                    )}
                    {step === 3 && (
                        <SharedFormStep3
                            ...
                            getValues={getValues}
                            onTemplateLoaded={() => { }}
                            templateName="AX9-B_Step3_Operations"
                            // 👇 Pass logic Approved ? Old : New
                            staticBlueprint={status === 'Approved' && templates ? templates['AX9-B_Step3_Operations'] : undefined}
                        />
                    )}
