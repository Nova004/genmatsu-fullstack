// location: frontend/src/components/formGen/pages/BZ3-B_Form/BZFormEdit.tsx
...
const BZ3_BFormEdit: React.FC<BZ3_BFormEditProps> = ({ initialData, onSubmit, onResubmit, status, templates }) => {
    const totalSteps = 4;
    ...
    const { step, setStep, handleNext, handleBack, handleSubmit_form } = useMultiStepForm({
        ...
        trigger,
        errors,
        validationSchema: BZ3_B_VALIDATION_SCHEMA,
    });


    // --- ค่าคงที่สำหรับ Styling และ Dropdown ---
    const availableForms = [{ value: 'G016', label: 'BZ3-B', path: '#' }]; // ไม่จำเป็นต้องมี path จริงในโหมดแก้ไข
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6">
            <form onSubmit={handleSubmit(handleFormSubmit)}>
                <FormHeader
                    title="แก้ไขใบรายงานการผลิต (BZ3-B)" // เปลี่ยน Title สำหรับหน้าแก้ไข
                    ...
                    */}
                    ...
                    {step === 2 && (
                        <FormStep2
                            ...
                            staticBlueprint={status === 'Approved' && templates ? templates['BZ3-B_Step2_RawMaterials'] : undefined}
                        />
                    )}
                    {step === 3 && (
                        <SharedFormStep3
                            ...
                            getValues={getValues}
                            onTemplateLoaded={() => { }}
                            templateName="BZ3-B_Step3_Operations"
                            // 👇 Pass logic Approved ? Old : New
                            staticBlueprint={status === 'Approved' && templates ? templates['BZ3-B_Step3_Operations'] : undefined}
                        />
                    )}
