// location: frontend/src/components/formGen/pages/BN_Form/BNFormEdit.tsx
...
const BNFormEdit: React.FC<BNFormEditProps> = ({ initialData, onSubmit, onResubmit, submissionId, status, templates }) => {
    ...
    const availableForms = [{ value: 'G011', label: 'BN', path: '#' }]; // ไม่จำเป็นต้องมี path จริงในโหมดแก้ไข
    ...
    return (
        <div className="rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6">
            <form onSubmit={handleSubmit(handleFormSubmit)}>
                <FormHeader
                    title="แก้ไขใบรายงานการผลิต (BN)" // เปลี่ยน Title สำหรับหน้าแก้ไข
                ...
                <div className="my-6">
                    ...
                    */}
                    {step === 1 && <SharedFormStep1 register={register} watch={watch} setValue={setValue} packagingWarningItemName="Zeolite (Z)" errors={errors} />}
                    {step === 2 && (
                        <FormStep2
                            register={register}
                            watch={watch}
                            ...
                            staticBlueprint={status === 'Approved' && templates ? templates['BN_Step2_RawMaterials'] : undefined}
                        />
                    )}
                    {step === 3 && (
                        <SharedFormStep3
                            ...
                            getValues={getValues}
                            onTemplateLoaded={() => { }}
                            templateName="BN_Step3_Operations"
                            // 👇 Pass logic Approved ? Old : New
                            staticBlueprint={status === 'Approved' && templates ? templates['BN_Step3_Operations'] : undefined}
                        />
                    )}
