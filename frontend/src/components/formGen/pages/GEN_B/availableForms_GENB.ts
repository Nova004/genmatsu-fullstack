
// location: frontend/src/components/formGen/pages/GEN_B/availableForms_GENB.ts
export const availableForms = [
    { value: 'BZ', label: 'BZ', path: '/forms/bz-form' },
    { value: 'BS-B', label: 'BS-B', path: '/forms/bs-b-form' },
    { value: 'BS', label: 'BS', path: '/forms/bs-form' },
    { value: 'BN', label: 'BN', path: '/forms/bn-form' },
    //{ value: 'BZ3', label: 'BZ3', path: '/forms/bz3-form' }, ยกเลิกแบบฟอร์ม BZ3
    { value: 'BZ3-B', label: 'BZ3-B', path: '/forms/bz3-b-form' },
    // { value: 'BS3', label: 'BS3', path: '/forms/bs3-form' }, ยกเลิกแบบฟอร์ม BS3
    { value: 'BS3-B', label: 'BS3-B', path: '/forms/bs3-b-form' },
    { value: 'BS3-B1', label: 'BS3-B1', path: '/forms/bs3-b1-form' },
    { value: 'BS3-C', label: 'BS3-C', path: '/forms/bs3-c-form' },
    { value: 'BZ5-C', label: 'BZ5-C', path: '/forms/bz5-c-form' },
    { value: 'BS5-C', label: 'BS5-C', path: '/forms/bs5-c-form' },

];

// คุณอาจจะ export ค่าคงที่อื่นๆ ที่เกี่ยวข้องไว้ในไฟล์นี้ด้วยก็ได้
export const DEFAULT_PATH = '/forms';